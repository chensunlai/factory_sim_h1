...
private:
  ...
  void laserCloudHandler(const sensor_msgs::msg::PointCloud2::SharedPtr laserCloudIn) const
  {
    ...
    for (const auto& field : laserCloudIn->fields) {
    ...
    if (has_intensity_field) {
      ...
      laserCloud->points.resize(laserCloudXYZ.points.size());
      laserCloud->width = laserCloudXYZ.width;
      laserCloud->height = laserCloudXYZ.height;
      laserCloud->is_dense = laserCloudXYZ.is_dense;
      for (std::size_t i = 0; i < laserCloudXYZ.points.size(); i++) {
        const auto& src = laserCloudXYZ.points[i];
        auto& dst = laserCloud->points[i];
        dst.x = src.x;
        dst.y = src.y;
        dst.z = src.z;
        dst.intensity = 255.0f;
      }
      RCLCPP_WARN_ONCE(
          this->get_logger(),
          "loamInterface: input cloud has no 'intensity' field. Falling back to constant intensity.");
    }
